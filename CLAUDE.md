...
Must be running on `http://localhost:1234` with a model loaded before starting sessions. Configured via `backend/.env` or the Settings panel. When `LLM_PROVIDER=mlx`, LM Studio is not required — inference runs via a managed `mlx_lm.server` subprocess.
...
Browser mic → AudioWorklet (Float32→Int16 PCM) → WebSocket binary frames
    ...
    → LLM (LM Studio or mlx_lm.server, streaming response) → Kokoro TTS (speech synthesis)
...
- `GET /api/settings` — Get current runtime configuration (VAD silence, LLM provider/model, MLX model, TTS voice, available voices/models, local MLX models from directory scan, model directories)
- `PATCH /api/settings` — Update runtime configuration (VAD silence 300-2000ms, LLM provider, LLM model, MLX model, MLX model directories, TTS voice). Switching `llm_provider` invalidates cached clients and shuts down mlx server if switching away from mlx. Changing `mlx_model` restarts the mlx server. Changes are in-memory only, not persisted to `.env`.
...
- **`conversation/llm.py`** — Pluggable LLM provider. `LLM_PROVIDER=lmstudio` (default) uses AsyncOpenAI client pointing to LM Studio. `LLM_PROVIDER=mlx` uses AsyncOpenAI client pointed at a managed `mlx_lm.server` subprocess (via `_get_mlx_client()`). Streaming `stream_chat_completion()` routes to the configured provider. `invalidate_caches()` resets the cached mlx client (called on provider/model change). Exports `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`, `MLX_MODEL`, `LLM_PROVIDER` — all mutable at runtime via settings API.
- **`conversation/mlx_server.py`** — Managed `mlx_lm.server` subprocess lifecycle. `ensure_mlx_server()` lazily starts the server on first use (polls `/v1/models` health endpoint, 60s timeout). `shutdown_mlx_server()` terminates the subprocess. Uses `atexit.register()` as safety net. Port configurable via `MLX_SERVER_PORT` env var (default 8642). Reads `MLX_MODEL` dynamically at start time (not import time) to support runtime model switching.
- **`conversation/model_scanner.py`** — Scans local directories for MLX-compatible LLM models. `MLX_MODEL_DIRS` (mutable list, from `MLX_MODEL_DIRS` env var + `~/.cache/huggingface/hub` default). `scan_local_models()` returns deduplicated, sorted list of discovered models. Supports HF cache layout (`models--org--name/snapshots/*/`, maxsplit=2 for `--` parsing) and standard one-level/two-level org/model directory layouts. Filters to LLM architectures only (`*ForCausalLM`, `*ForConditionalGeneration`) via `config.json` — skips embedding, vision, and audio models. Skips GGUF-only directories.
...
- **`conversation/router.py`** — Lightweight LLM-based phase transition router. Runs off the hot path after each bot response. Uses `PhaseDecision` Pydantic model with `should_advance`, `next_phase`, `reasoning`. Force-advances at `max_turns`. `_get_client()` lazily resolves the OpenAI client based on `LLM_PROVIDER` (LM Studio client or mlx server client). `invalidate_caches()` resets the cached client on provider/model change.
- **`conversation/feedback.py`** — Post-session analysis using Pydantic AI agents for type-safe structured output. Agents created without a model — `_get_model()` lazily resolves the Pydantic AI model based on `LLM_PROVIDER` (OpenAI endpoint for LM Studio, or mlx server endpoint). Model passed at `.run()` time. `invalidate_caches()` resets the cached model on provider/model change. `count_filler_words()` uses regex on user transcripts. `generate_rubric_feedback()` runs multi-pass per-phase evaluation with rubric anchoring (3/5/7/9 levels per focus area), transcript evidence quotes, stronger-answer suggestions, and diagram context injection (serialized tldraw text per phase). `generate_feedback_legacy()` provides backward-compatible single-call evaluation for scenarios without rubrics. Uses `DimensionScore`, `PhaseEvaluationResult`, `SummaryResult`, and `LegacyFeedbackResult` Pydantic output models with `pydantic_ai.Agent`. Optionally grounds evaluation against RAG-retrieved technical reference material.
...
- **`src/stores/settingsStore.ts`** — Zustand store for runtime settings. `Settings` interface includes `llm_provider`, `mlx_model`, `available_mlx_models` (HF presets from backend), `local_mlx_models` (discovered local models), and `mlx_model_dirs`. `fetchSettings()` loads from `GET /api/settings`. `updateSettings()` patches via `PATCH /api/settings` — accepts `llm_provider`, `mlx_model`, and `mlx_model_dirs` alongside existing fields.
...
- Backend `.env` is gitignored; `backend/.env` holds runtime config (VAD_SILENCE_MS, LLM_PROVIDER, LLM_BASE_URL, LLM_API_KEY, LLM_MODEL, MLX_MODEL, MLX_MODEL_DIRS, MLX_SERVER_PORT, KOKORO_VOICE, EMBEDDING_PROVIDER, FASTEMBED_MODEL, OLLAMA_BASE_URL, EMBEDDING_MODEL)
...
- LLM provider is pluggable: `LLM_PROVIDER=lmstudio` (default, OpenAI-compatible API) or `mlx` (managed `mlx_lm.server` subprocess on port 8642). All inference paths (live conversation, feedback agents, phase router) route through the same backend — LM Studio API or mlx server. Provider and model are switchable at runtime via `PATCH /api/settings` (invalidates cached clients, restarts mlx server on model change).
...
- Runtime settings (VAD, LLM provider, LLM model, MLX model, MLX model dirs, TTS voice) are mutable module-level variables, changed via `PATCH /api/settings` — not persisted to `.env`. Provider/model changes trigger `invalidate_caches()` on `llm.py`, `feedback.py`, and `router.py` to reset cached clients/models.
- MLX model directories are scanned by `model_scanner.py` for local models. Default scan path is `~/.cache/huggingface/hub`. Additional paths configurable via `MLX_MODEL_DIRS` env var (comma-separated) or the Settings panel. Scanner supports HF cache layout and one/two-level org/model layouts. Filters to LLM architectures only (CausalLM/ConditionalGeneration).
